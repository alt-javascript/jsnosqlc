...
describe('DriverManager', () => {
  ...
  it('registers a driver', () => {
    const d = new StubDriver('jsnoslqc:stub:');
  ...
  it('getClient routes to the correct driver', async () => {
    const dA = new StubDriver('jsnoslqc:a:');
    const dB = new StubDriver('jsnoslqc:b:');
    DriverManager.registerDriver(dA);
    DriverManager.registerDriver(dB);
    const client = await DriverManager.getClient('jsnoslqc:b:test');
    assert.instanceOf(client, StubClient);
    assert.equal(client.getUrl(), 'jsnoslqc:b:test');
  });

  it('getClient throws when no driver matches', async () => {
    DriverManager.registerDriver(new StubDriver('jsnoslqc:x:'));
    await assert.isRejected
      ? assert.isRejected(DriverManager.getClient('jsnoslqc:unknown:test'))
      : await DriverManager.getClient('jsnoslqc:unknown:test').then(
  ...
  it('clear resets the registry', () => {
    DriverManager.registerDriver(new StubDriver('jsnoslqc:a:'));
    DriverManager.clear();
    assert.deepEqual(DriverManager.getDrivers(), []);
  });

  it('getDrivers returns a copy, not the internal array', () => {
    const d = new StubDriver('jsnoslqc:a:');
    DriverManager.registerDriver(d);
